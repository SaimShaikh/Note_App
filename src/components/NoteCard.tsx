...
const NoteCard: React.FC<NoteCardProps> = ({ note, onEdit, onDelete }) => {
  ...
  return (
    ...
    >
      <div className="bg-white/70 backdrop-blur-md border border-white/20 rounded-2xl p-6 shadow-xl hover:shadow-2xl transition-all duration-300 h-full flex flex-col">
        {/* Header */}
        ...
        {/* Content */}
        <p className="text-gray-600 line-clamp-4 flex-1 mb-4">
          {note.content}
        </p>
        ...
        {note.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {note.tags.map((tag, index) => (
              <span
                key={index}
                className={`px-2 py-1 rounded-full text-xs font-medium ${getTagColor(index)} flex items-center gap-1`}
              >
                <Tag size={10} />
                {tag}
              </span>
